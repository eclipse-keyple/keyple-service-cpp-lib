 ...
 * @since 2.0.0
 */
class KEYPLESERVICE_API ObservablePlugin : virtual public Plugin {
public:
    /**
     * Registers a new observer to be notified when a plugin event occurs.
     *
     * <p>The provided observer must implement the PluginObserverSpi interface
     * to be able to receive the events produced by this plugin (reader
     * connection, disconnection).
     *
     * <p>If applicable, the observation process shall be started when the first
     * observer is added.
     *
     ...
     */
    virtual void addObserver(const std::shared_ptr<PluginObserverSpi> observer)
        = 0;

    /**
     * Unregisters a plugin observer.
     *
     * <p>The observer will no longer receive any of the events produced by the
     * plugin.
     *
     * <p>If applicable, the observation process shall be stopped when the last
     * observer is removed.
     *
     * @param observer The observer object to be unregistered (should be not
     * null).
     * @since 2.0.0
     * @throw IllegalArgumentException if observer is null.
     */
    virtual void
    removeObserver(const std::shared_ptr<PluginObserverSpi> observer)
        = 0;
