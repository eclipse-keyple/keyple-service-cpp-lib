...
std::shared_ptr<ConfigurableReaderSpiMock> ReaderAdapterTestUtils::getReaderSpiSpy()
{
    ...
    EXPECT_CALL(*readerSpiSpy.get(), transmitApdu(_)).WillRepeatedly(Return(HexUtil::toByteArray("6D00")));
...
std::shared_ptr<ObservableControllableReaderSpiMock> ReaderAdapterTestUtils::getObservableReaderSpi()
{
    ...
    EXPECT_CALL(*observableReaderSpi.get(), transmitApdu(_)).WillRepeatedly(Return(HexUtil::toByteArray("6D00")));
