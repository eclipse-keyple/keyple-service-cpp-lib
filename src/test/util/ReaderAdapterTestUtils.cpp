...
/* Mock */
#include "mock/ApduResponseApiMock.hpp"
...
const std::string ReaderAdapterTestUtils::OTHER_CARD_PROTOCOL
    = "otherCardProtocol";
...
, MULTI_NOT_MATCHING_RESPONSES(
      {getNotMatchingResponse(), getNotMatchingResponse()})
, MATCHING_RESPONSES({getMatchingResponse()})
, MULTI_MATCHING_RESPONSES(
      {getNotMatchingResponse(), getMatchingResponse(), getMatchingResponse()})
{
}
...
std::shared_ptr<ConfigurableReaderSpiMock>
ReaderAdapterTestUtils::getReaderSpi()
{
...
std::shared_ptr<ConfigurableReaderSpiMock>
ReaderAdapterTestUtils::getReaderSpiSpy()
{
    readerSpiSpy = std::make_shared<ConfigurableReaderSpiMock>();
    EXPECT_CALL(*readerSpiSpy.get(), getName())
        .WillRepeatedly(ReturnRef(READER_NAME));
    EXPECT_CALL(*readerSpiSpy.get(), checkCardPresence())
        .WillRepeatedly(Return(true));
    EXPECT_CALL(*readerSpiSpy.get(), getPowerOnData())
        .WillRepeatedly(Return(POWER_ON_DATA));
    EXPECT_CALL(*readerSpiSpy.get(), transmitApdu(_))
        .WillRepeatedly(Return(HexUtil::toByteArray("6D00")));
    EXPECT_CALL(*readerSpiSpy.get(), isProtocolSupported(CARD_PROTOCOL))
        .WillRepeatedly(Return(true));
    EXPECT_CALL(*readerSpiSpy.get(), isCurrentProtocol(CARD_PROTOCOL))
        .WillRepeatedly(Return(true));
...
ReaderAdapterTestUtils::getObservableReaderSpi()
{
    observableReaderSpi
        = std::make_shared<ObservableControllableReaderSpiMock>();
    EXPECT_CALL(*observableReaderSpi.get(), getName())
        .WillRepeatedly(ReturnRef(READER_NAME));
    EXPECT_CALL(*observableReaderSpi.get(), checkCardPresence())
        .WillRepeatedly(Return(true));
    EXPECT_CALL(*observableReaderSpi.get(), getPowerOnData())
        .WillRepeatedly(Return(POWER_ON_DATA));
    EXPECT_CALL(*observableReaderSpi.get(), transmitApdu(_))
        .WillRepeatedly(Return(HexUtil::toByteArray("6D00")));
    EXPECT_CALL(*observableReaderSpi.get(), isProtocolSupported(CARD_PROTOCOL))
        .WillRepeatedly(Return(true));
    EXPECT_CALL(*observableReaderSpi.get(), isCurrentProtocol(CARD_PROTOCOL))
        .WillRepeatedly(Return(true));

    return observableReaderSpi;
}

std::shared_ptr<CardSelectionResponseApiMock>
ReaderAdapterTestUtils::getMatchingResponse()
{
    matching = std::make_shared<CardSelectionResponseApiMock>();
    EXPECT_CALL(*matching.get(), hasMatched()).WillRepeatedly(Return(true));
    EXPECT_CALL(*matching.get(), getCardResponse())
        .WillRepeatedly(Return(std::make_shared<CardResponseApiMock>()));
    EXPECT_CALL(*matching.get(), getSelectApplicationResponse())
        .WillRepeatedly(Return(std::make_shared<ApduResponseApiMock>()));

    return matching;
}

std::shared_ptr<CardSelectionResponseApiMock>
ReaderAdapterTestUtils::getNotMatchingResponse()
{
    ...
    EXPECT_CALL(*notMatching.get(), getCardResponse())
        .WillRepeatedly(Return(std::make_shared<CardResponseApiMock>()));
    EXPECT_CALL(*notMatching.get(), getSelectApplicationResponse())
        .WillRepeatedly(Return(std::make_shared<ApduResponseApiMock>()));
